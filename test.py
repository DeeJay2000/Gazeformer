...
def run_model(
    model, src, task, device="cuda:0", im_h=20, im_w=32, patch_size=16, num_samples=1
):
    src = src.to(device).repeat(num_samples, 1, 1)
    task = (
        torch.tensor(task.astype(np.float32))
        .to(device)
        .unsqueeze(0)
        .repeat(num_samples, 1)
    )
    firstfix = (
        torch.tensor([(im_h // 2) * patch_size, (im_w // 2) * patch_size])
        .unsqueeze(0)
        .repeat(num_samples, 1)
    )
    with torch.no_grad():
...
def test(args):
    ...
    test_target_trajs = list(
        filter(
            lambda x: x["split"] == "test" and x["condition"] == args.condition,
            human_scanpaths,
        )
    )
    if args.zerogaze:
        test_target_trajs = list(
            filter(
                lambda x: x["task"] == args.task.replace("_", " "), test_target_trajs
            )
        )
    ...
    for traj in test_target_trajs:
        key = "test-{}-{}-{}-{}".format(
            traj["condition"], traj["task"], traj["name"][:-4], traj["subject"]
        )
    ...
    embedding_dict = np.load(
        open(join(dataset_root, "embeddings.npy"), mode="rb"), allow_pickle=True
    ).item()
    pred_list = []
    ...
    for target_traj in tqdm(test_task_img_pairs):
        ...
        image_ftrs = torch.load(
            join(img_ftrs_dir, task_name.replace(" ", "_"), name.replace("jpg", "pth"))
        ).unsqueeze(0)
...
def main(args):
    ...
    print(
        "Sequence Score : {:.3f}, Sequence Score with Duration : {:.3f}".format(
            seq_score, seq_score_t
        )
    )
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        "Gaze Transformer Test", parents=[get_args_parser_test()]
    )
